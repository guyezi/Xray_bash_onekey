...
guyezi_conf_dir="${guyezi_dir}/conf"
xray_conf_dir="${guyezi_conf_dir}/xray"
nginx_conf_dir="${guyezi_conf_dir}/nginx"
...
guyezi_tmp="${guyezi_dir}/tmp"
guyezi_commend_file="/usr/bin/guyezi"
ssl_chainpath="${guyezi_dir}/cert"
nginx_dir="/etc/nginx"
nginx_openssl_src="/usr/local/src"
xray_bin_dir="/usr/local/bin/xray"
xray_info_file="${guyezi_dir}/info/xray_info.inf"
xray_qr_config_file="${guyezi_dir}/info/vless_qr.json"
...
create_directory() {
    ...
    [[ ! -d "${guyezi_dir}/info" ]] && mkdir -p ${guyezi_dir}/info
    [[ ! -d "${guyezi_tmp}" ]] && mkdir -p ${guyezi_tmp}
}
...
ssl_install() {
    ...
    read_optimize "请输入注册域名的邮箱 (eg:me@guyezi.com):" "myemail" "NULL"
...
domain_check() {
    ...
    read_optimize "请输入你的域名信息 (eg:yun.guyezi.com):" "domain" "NULL"
...
xray_conf_add() {
    ...
    if [[ ${tls_mode} == "TLS" ]]; then
        wget --no-check-certificate https://raw.githubusercontent.com/guyezi/Xray_bash_onekey/main/VLESS_tls/config.json -O config.json
        modify_path
        modify_inbound_port
    elif [[ ${tls_mode} == "XTLS" ]]; then
        wget --no-check-certificate https://raw.githubusercontent.com/guyezi/Xray_bash_onekey/main/VLESS_xtls/config.json -O config.json
        xray_xtls_add_more
    elif [[ ${tls_mode} == "None" ]]; then
        wget --no-check-certificate https://raw.githubusercontent.com/guyezi/Xray_bash_onekey/main/VLESS_tls/config.json -O config.json
        modify_listen_address
...
nginx_conf_add() {
    ...
    server {
        ...
        listen [::]:443 ssl http2;
        ssl_certificate       /etc/guyezi/cert/xray.crt;
        ssl_certificate_key   /etc/guyezi/cert/xray.key;
        ...
        root /400.html;
        error_page 400 https://yun.guyezi.com;
...
acme_cron_update() {
    ...
    *)
    ...
    [yY][eE][sS] | [yY])
        if [[ "${ssl_self}" != "on" ]]; then
            wget -N -P ${guyezi_dir} --no-check-certificate https://raw.githubusercontent.com/guyezi/Xray_bash_onekey/main/ssl_update.sh && chmod +x ${ssl_update_file}
...
bbr_boost_sh() {
    if [[ -f "${guyezi_dir}/tcp.sh" ]]; then
        cd ${guyezi_dir} && chmod +x ./tcp.sh && ./tcp.sh
    else
        wget -N --no-check-certificate -P ${guyezi_dir} "https://raw.githubusercontent.com/ylx2016/Linux-NetSpeed/master/tcp.sh" && chmod +x ${guyezi_dir}/tcp.sh && ${guyezi_dir}/tcp.sh
    fi
}
...
update_sh() {
    ol_version=$(curl -L -s https://raw.githubusercontent.com/guyezi/Xray_bash_onekey/main/install.sh | grep "shell_version=" | head -1 | awk -F '=|"' '{print $3}')
    echo "${ol_version}" >${version_cmp}
    [[ -z ${ol_version} ]] && clear && echo -e "${Error} ${RedBG}  检测最新版本失败! ${Font}" && bash guyezi
    ...
    if [[ ${shell_version} != ${newest_version} ]]; then
        ...
        [yY][eE][sS] | [yY])
            [[ -L ${guyezi_commend_file} ]] && rm -f ${guyezi_commend_file}
            wget -N --no-check-certificate -P ${guyezi_dir} https://raw.githubusercontent.com/guyezi/Xray_bash_onekey/main/install.sh && chmod +x ${guyezi_dir}/install.sh
            ln -s ${guyezi_dir}/install.sh ${guyezi_commend_file}
            clear
...
guyezi_commend() {
    if [[ -L ${guyezi_commend_file} ]] || [[ -f ${guyezi_dir}/install.sh ]]; then
        old_version=$(grep "shell_version=" ${guyezi_dir}/install.sh | head -1 | awk -F '=|"' '{print $3}')
        ...
        if [[ -z ${old_version} ]]; then
            ...
            bash guyezi
        elif [[ ! -L ${guyezi_commend_file} ]]; then
            ln -s ${guyezi_dir}/install.sh ${guyezi_commend_file}
            echo -e "${Green}可以使用${Red} guyezi ${Font}命令管理脚本\n${Font}"
        else
            echo -e "${Green}可以使用${Red} guyezi ${Font}命令管理脚本\n${Font}"
        fi
    else
        ...
        pkg_install "bc,wget"
        wait
        [[ ! -d "${guyezi_dir}" ]] && mkdir -p ${guyezi_dir}
        wget -N --no-check-certificate -P ${guyezi_dir} https://raw.githubusercontent.com/guyezi/Xray_bash_onekey/main/install.sh && chmod +x ${guyezi_dir}/install.sh
        judge "下载最新脚本"
        ln -s ${guyezi_dir}/install.sh ${guyezi_commend_file}
        clear
