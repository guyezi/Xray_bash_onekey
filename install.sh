...
ssl_update_file="${gyz_xray_dir}/ssl_update.sh"
gyz_commend_file="/usr/bin/gyz"
...
modify_nginx_other() {
    ...
    sed -i "/returc/c \\\t\\treturn 302 https://www.guyezi.com/helloworld;" ${nginx_conf}
...
domain_check() {
    read -rp "请输入你的域名信息(eg:yun.guyezi.com):" domain
...
acme() {
    ...
    if "$HOME"/.acme.sh/acme.sh --issue -d "${domain}" --standalone -k ec-256 --force; then
        ...
        sleep 2
        mkdir /data
        if "$HOME"/.acme.sh/acme.sh --installcert -d "${domain}" --fullchainpath /cert/xray.crt --keypath /cert/xray.key --ecc --force; then
            chmod -f a+rw /cert/xray.crt
            chmod -f a+rw /cert/xray.key
            chown -f nobody:nobody /cert/xray.crt
            chown -f nobody:nobody /cert/xray.key
...
nginx_conf_add() {
    ...
    server {
        ...
        #root  /www;
        root /400.html;
        error_page 400 https://www.guyezi.com/helloworld;
...
acme_cron_update() {
    wget -N -P /usr/bin/gyz-xray --no-check-certificate "https://raw.githubusercontent.com/guyezi/Xray_bash_onekey/main/ssl_update.sh"
...
update_sh() {
    ...
    if [[ "$shell_version" < "$(sort -rV $version_cmp | head -1)" ]]; then
        ...
        [yY][eE][sS] | [yY])
            rm -f ${gyz_commend_file}
            wget -N --no-check-certificate -P ${gyz_xray_dir} https://raw.githubusercontent.com/guyezi/Xray_bash_onekey/main/install.sh && chmod +x ${gyz_xray_dir}/install.sh
            ln -s ${gyz_xray_dir}/install.sh ${gyz_commend_file}
...
gyz_commend() {
    #增加管理命令
    if [ -L "${gyz_commend_file}" ]; then
        echo -e "${Green}可以使用${Red}gyz${Font}命令管理脚本\n${Font}"
    else
        if [ -L "/usr/local/bin/gyz" ]; then
            rm -f /usr/local/bin/gyz
        fi
        ln -s $(
            cd "$(dirname "$0")"
            pwd
        )/install.sh ${gyz_commend_file}
        echo -e "${Green}可以使用${Red}gyz${Font}命令管理脚本\n${Font}"
    fi
}
